...
exports.handler = async (event) => {
  try {
    ...
    const rows = await supaFetch(
      `videos?sender_id=eq.${user.id}&select=id,display_name,message,size,uploaded_at,receiver:receiver_id(username)&order=uploaded_at.desc`
    );
    const mapped = (rows || []).map((r) => ({
      ...
      to_user: r.receiver ? r.receiver.username : '?'
    }));
