...
app.post('/api/send', requireAuth, upload.single('video'), (req, res) => {
  ...
  db.prepare(`INSERT INTO videos (sender_id, receiver_id, filename, original_name, size)
              VALUES (?, ?, ?, ?, ?)`)
    .run(req.session.userId, receiver.id, req.file.filename, req.file.originalname, req.file.size);
...
app.get('/api/inbox', requireAuth, (req, res) => {
  const rows = db.prepare(`
    SELECT videos.id, videos.original_name, videos.size, videos.uploaded_at, users.username as from_user
...
app.get('/api/sent', requireAuth, (req, res) => {
  const rows = db.prepare(`
    SELECT videos.id, videos.original_name, videos.size, videos.uploaded_at, users.username as to_user
